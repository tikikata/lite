...
echo ""
echo -n > /etc/wireguard/clients.txt
data=( `cat /etc/wireguard/wgrex.conf | grep "### Client" | awk '{ print $3 }'`);
hr(){
...
for akun in "${data[@]}"
do
pub=$(cat /etc/wireguard/wgrex.conf | grep PublicKey | awk '{ print $3 }' | tr '\n' ' ' | awk '{print $'"$x"'}')
...
listClients(){
    if DUMP="$(wg show wgrex dump)"; then
        DUMP="$(tail -n +2 <<< "$DUMP")"
    else
        exit 1
    fi
